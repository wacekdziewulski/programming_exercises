...
def processExpression(expression, begin):
	lhs = None
	operator = None
	i = begin
	while i < len(expression):
		...
		if char in list(string.digits):
			...
			(partial_result, new_position) = processExpression(expression, i+1)
			if not lhs:
				lhs = partial_result
			if operator == '+':
				lhs = lhs + int(partial_result)
			elif operator == '*':
				lhs = lhs * int(partial_result)
			i = new_position
			continue
...
for line in input.split('\n'):
	line = line.replace(' ', '')
	result = processExpression(line, 0)[0]
	sum_all += result
	print(result)
